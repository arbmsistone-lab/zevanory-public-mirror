  ...
  const outcome=await executeStorageMutation({operation,mutate:async()=>{
    ...
    const rows=await sql.query(`
      with inserted_event as (
        insert into telemetry_events(event_id,event_name,session_id,experiment_id,offer_id,channel,source,payload)
        ...
        select $9,$3,$6,'new',0,now() where $2='cta_whatsapp' and exists(select 1 from inserted_event)
      ...
      select $10,'lead_review','queued',80,lead_id,$11,$12::jsonb,now() from lead_upsert
      on conflict(idempotency_key) do nothing returning job_id
    `,[event.event_id,event.event_name,event.session_id,event.experiment_id,event.offer_id,event.channel,event.source,JSON.stringify(event.payload||{}),randomUUID(),randomUUID(),`public-cta:${event.session_id}`,JSON.stringify({source:'public_event',event_name:event.event_name})]);
