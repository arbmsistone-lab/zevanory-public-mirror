...
export default async function handler(req,res){
  ...
  try{
    ...
    const [runs,tools,jobs,outbox,actions]=await Promise.all([
      sql.query(`select r.run_id,r.job_id,j.job_type,r.provider,r.model,r.mode,r.outcome,r.latency_ms,r.created_at,
        r.decision->>'action' action,r.decision->>'rationale' rationale,r.decision->>'confidence' confidence,
        r.decision->>'tool' tool,r.decision->'result' result,r.decision->'eval' eval
      ...
      sql.query(`select job_id,job_type,status,priority,attempts,available_at,locked_at,completed_at,last_error,created_at
        from agent_jobs order by created_at desc limit $1`,[limit]),
      sql.query(`select event_id,aggregate_type,event_type,destination,status,attempts,available_at,locked_at,delivered_at,last_error,created_at
        from integration_outbox order by created_at desc limit $1`,[limit]),
    ...
    return json(res,200,{mode:'operator',generated_at:new Date().toISOString(),gates,channels,runs:runs.map(x=>({...x,rationale:String(x.rationale||'').slice(0,500)})),tools,jobs:jobs.map(x=>({...x,last_error:cleanError(x.last_error)})),outbox:outbox.map(x=>({...x,last_error:cleanError(x.last_error)})),actions});
