...
test('WhatsApp adapter requires channel gate and returns provider id only',async()=>{
  ...
  const adapters=buildOutboundAdapters({env,commercialGate:certifiedGate,fetchImpl:async(url,options)=>{calls.push({url,options});return response(200,{messages:[{id:'wamid.1'}]});}});
...
test('Resend adapter sends with idempotency and does not claim delivery truth',async()=>{
  ...
  const adapters=buildOutboundAdapters({env,commercialGate:certifiedGate,fetchImpl:async(url,options)=>{calls.push({url,options});return response(200,{id:'email_1'});}});
...
test('Instagram requires HTTPS media and publishes through container',async()=>{
  ...
  const adapters=buildOutboundAdapters({env,commercialGate:certifiedGate,fetchImpl:async(url,options)=>{calls.push({url,options});return calls.length===1?response(200,{id:'container1'}):response(200,{id:'media1'});}});
