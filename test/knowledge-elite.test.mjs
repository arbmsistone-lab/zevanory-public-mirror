...
test('knowledge query uses broad OR-prefix retrieval for natural customer language',async()=>{
  ...
  await searchKnowledge(sql,'Quero saber como funciona o ARBM ONE no salao de beleza',10);
...
test('knowledge seed covers company ARBM SIST ARBM ONE and all five certified content products',()=>{
  ...
  for(const marker of ['ZEVANORY identidade e posicionamento','ZEVANORY-PRODUCTS-V21-HANDOFF.md','product:ARBM-SIST','products/support/','products/releases/${productRelease}/','active=false'])assert.ok(s.includes(marker),marker);
