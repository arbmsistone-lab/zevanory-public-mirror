...
test('single-screen primary surface keeps current executive domains visible',()=>{
  for(const name of ['Centro executivo','Resumo executivo','Operação ao vivo','Prontidão comercial','Confiabilidade &amp; excelência','Garantias']) assert.ok(html.includes(name));
...
test('desktop primary surface stays scroll-free with explicit detail disclosure',()=>{
  ...
  assert.match(css,/grid-template-columns:minmax\(300px,\.9fr\) minmax\(500px,1\.35fr\) minmax\(320px,\.95fr\)/);
