...
function fakeSql({paused=false}={}){
  ...
  return {state,async query(text,args=[]){
    ...
    if(q.includes('from agent_control_state')) return [{paused,reason:paused?'operator_pause':'ready',changed_by:'test',changed_at:null}];
    ...
    if(q.includes("update agent_runs set outcome='blocked'")) return [];
    if(q.includes('update agent_runs set outcome=$2')) return [];
...
test('agent persists run before FK-backed tool audit',async()=>{
  ...
  assert.equal(result.ok,true); assert.equal(result.outcome,'completed');
  assert.equal(sql.state.runInserted,true); assert.equal(sql.state.auditInserted,true); assert.equal(sql.state.jobStatus,'completed');
});

test('safe pause blocks the worker before claiming a job',async()=>{
  const sql=fakeSql({paused:true}); const result=await runAgentOnce(sql,{env:{AGENT_AI_ENABLED:'false'}});
  assert.equal(result.processed,false); assert.equal(result.reason,'agent_paused');
  assert.equal(sql.state.queries.some(q=>q.includes("update agent_jobs set status='running'")),false);
});
