...
test('TikTok queries creator and forces SELF_ONLY for unaudited client',async()=>{
  ...
  const a=buildOutboundAdapters({env,fetchImpl:async(url,opt)=>{calls.push({url,opt});return calls.length===1?response(200,{data:{privacy_level_options:['SELF_ONLY','PUBLIC_TO_EVERYONE']},error:{code:'ok'}}):response(200,{data:{publish_id:'pub-1'},error:{code:'ok'}});}});
...
test('LinkedIn posts through official REST Posts API and requires provider id',async()=>{
  ...
  const a=buildOutboundAdapters({env,fetchImpl:async(url,opt)=>{calls.push({url,opt});return response(201,{}, {'x-restli-id':'urn:li:share:1'});}});
...
test('affiliate adapter is generic, HTTPS-only and idempotent',async()=>{
  ...
  const a=buildOutboundAdapters({env,fetchImpl:async(url,opt)=>{calls.push({url,opt});return response(202,{tracking_id:'trk-1'});}});
