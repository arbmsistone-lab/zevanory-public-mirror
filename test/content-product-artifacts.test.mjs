...
const root=new URL('../products/releases/v2.1/',import.meta.url);
const sha256=bytes=>crypto.createHash('sha256').update(bytes).digest('hex');

test('all five content products have real v2.1 Master/Senior ZIP artifacts with canonical hashes',()=>{
  ...
  for(const product of ZEVANORY_PRODUCTS){
    ...
    assert.equal(product.version,'2.1');
