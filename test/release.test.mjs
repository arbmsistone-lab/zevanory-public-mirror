...
test('release fingerprint is canonical and immutable',()=>{
  assert.equal(RELEASE.id,'ZEVANORY-EG0014-RC1');
  assert.equal(Object.isFrozen(RELEASE),true);
  assert.equal(RELEASE.checkoutMode,'sandbox-disabled');
});
...
test('release endpoint exposes exact fingerprint and rejects writes',()=>{
  ...
  assert.equal(body.release_id,'ZEVANORY-EG0014-RC1');
  assert.equal(body.checkout_mode,'sandbox-disabled');
