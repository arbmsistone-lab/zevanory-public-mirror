...
const gateEvaluator=()=>({enabled:true});
...
test('guarded mode blocks commercial autonomy even with otherwise healthy evidence',()=>{
  const result=assessAutonomy({gateEvaluator,tool:'send_message',riskLevel:'commercial',env:{...env,AGENT_AUTONOMY_MODE:'guarded'},evalResult,context:{lead:{channel:'whatsapp'},outcome_learning:learning},health:healthy});
  assert.equal(result.eligible,false);assert.equal(result.reason,'guarded_mode');
});

test('refund is never autonomous in v1',()=>{
  const result=assessAutonomy({gateEvaluator,tool:'refund_payment',riskLevel:'financial',env,evalResult,context:{lead:{channel:'whatsapp'},outcome_learning:learning},health:healthy});
...
test('commercial autonomy requires learned winning channel and healthy runtime',()=>{
  const mismatch=assessAutonomy({gateEvaluator,tool:'send_message',riskLevel:'commercial',env,evalResult,context:{lead:{channel:'email'},outcome_learning:learning},health:healthy});
  assert.equal(mismatch.eligible,false);assert.equal(mismatch.reason,'learning_channel_mismatch');
  const ready=assessAutonomy({gateEvaluator,tool:'send_message',riskLevel:'commercial',env,evalResult,context:{lead:{channel:'whatsapp'},outcome_learning:learning},health:healthy});
...
test('runtime regressions revoke autonomy immediately',()=>{
  const retry=assessAutonomy({gateEvaluator,tool:'send_message',riskLevel:'commercial',env,evalResult,context:{lead:{channel:'whatsapp'},outcome_learning:learning},health:{...healthy,retry_24h:1}});
  assert.equal(retry.eligible,false);assert.equal(retry.reason,'integration_health_not_clean');
  const lowCoverage=assessAutonomy({gateEvaluator,tool:'send_message',riskLevel:'commercial',env,evalResult,context:{lead:{channel:'whatsapp'},outcome_learning:learning},health:{...healthy,eval_total_24h:90,eval_passed_24h:90}});
...
test('checkout may be autonomous only with explicit financial preconditions',()=>{
  const blocked=assessAutonomy({gateEvaluator,tool:'start_checkout',riskLevel:'financial',env:{...env,PAYMENT_MERCHANT_IDENTITY_VERIFIED:'false'},evalResult,context:{lead:{channel:'whatsapp'},outcome_learning:learning},health:healthy});
  assert.equal(blocked.eligible,false);assert.equal(blocked.reason,'financial_preconditions_closed');
  const ready=assessAutonomy({gateEvaluator,tool:'start_checkout',riskLevel:'financial',env,evalResult,context:{lead:{channel:'whatsapp'},outcome_learning:learning},health:healthy});
