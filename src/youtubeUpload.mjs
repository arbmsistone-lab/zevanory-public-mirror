...
export async function materializeRemoteVideo({url,fetchImpl=globalThis.fetch,maxBytes=MAX_SOURCE_BYTES}={}){
  const mediaUrl=httpsUrl(url,'youtube_media_url_required');
  const response=await fetchImpl(mediaUrl,{method:'GET',redirect:'follow'});
  if(!response.ok)throw new Error(`youtube_media_fetch_${response.status}`);
  ...
  if(Number.isFinite(declared)&&declared>maxBytes)throw new Error('youtube_media_too_large');
  const bytes=Buffer.from(await response.arrayBuffer());
  if(!bytes.length)throw new Error('youtube_media_size_missing');
  if(bytes.length>maxBytes)throw new Error('youtube_media_too_large');
  return Object.freeze({url:mediaUrl,size:bytes.length,mime,bytes,sha256:createHash('sha256').update(bytes).digest('hex')});
}
...
export async function uploadYouTubeFromRemote({event,sql,env=process.env,fetchImpl=globalThis.fetch}={}){
  ...
  let media=null;
  if(!state){
    media=await materializeRemoteVideo({url:event?.payload?.media_url,fetchImpl});
    state=await createYouTubeUploadSession({accessToken,video:media,title:event?.payload?.title||event?.payload?.content,description:event?.payload?.description||event?.payload?.content,privacyStatus:event?.payload?.privacy_status||env.YOUTUBE_PRIVACY_STATUS||'private',madeForKids:event?.payload?.made_for_kids===true,fetchImpl});
    state={...state,source_sha256:media.sha256};
    ...
    media=await materializeRemoteVideo({url:state.media_url,fetchImpl});
    if(media.size!==Number(state.total_bytes)||media.mime!==state.mime||!state.source_sha256||media.sha256!==state.source_sha256)throw new Error('youtube_media_changed_manual_reconcile');
  }
  if(!media)media=await materializeRemoteVideo({url:state.media_url,fetchImpl});
  let chunks=0;
  while(Number(state.uploaded_bytes)<Number(state.total_bytes)&&chunks<MAX_CHUNKS_PER_RUN){
    ...
    if(bytes.length!==expected)throw new Error('youtube_media_range_length_mismatch');
