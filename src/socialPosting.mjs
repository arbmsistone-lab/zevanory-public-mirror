...
export async function publishTikTok({event,env=process.env,fetchImpl=globalThis.fetch,accessToken}={}){
  ...
  const body={post_info:{title:clean(event?.payload?.content||event?.payload?.title,2200),privacy_level:privacy,disable_duet:true,disable_comment:false,disable_stitch:true},source_info:{source:'PULL_FROM_URL',video_url:mediaUrl}};
