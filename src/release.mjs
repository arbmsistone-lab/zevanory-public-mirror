...
export function runtimeReleaseModes(env = process.env) {
  const salesMode = yes(env.SALE_GLOBALLY_ENABLED) && yes(env.PRE_SALE_GATES_APPROVED) ? 'enabled' : 'globally-blocked';
  return Object.freeze({
    ...
    whatsappMode: salesMode === 'enabled' && yes(env.WHATSAPP_SALES_ENABLED) ? 'enabled' : 'disabled',
  });
}
