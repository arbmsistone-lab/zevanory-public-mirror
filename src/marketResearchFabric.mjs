...
export function marketResearchReadiness(env=process.env){
  ...
  const native={gdelt:true,youtube:Boolean(env.YOUTUBE_OAUTH_CLIENT_ID&&env.YOUTUBE_OAUTH_CLIENT_SECRET&&env.YOUTUBE_OAUTH_REFRESH_TOKEN),mercado_livre:Boolean(env.MERCADOLIVRE_APP_ID&&env.MERCADOLIVRE_CLIENT_SECRET&&env.MERCADOLIVRE_TOKEN_ENCRYPTION_KEY)};
