...
export async function verifyYouTubeIdentity({env=process.env,fetchImpl=globalThis.fetch}={}){
  ...
  if(!hasDirect&&!hasRefresh){
    try{const r=await fetchImpl(`https://www.youtube.com/feeds/videos.xml?channel_id=${EXPECTED_YOUTUBE_CHANNEL_ID}`,{signal:AbortSignal.timeout(10000)});const body=r.ok?await r.text():'';const feedChannelId=EXPECTED_YOUTUBE_CHANNEL_ID.replace(/^UC/,'');const channelOk=r.ok&&body.includes(`<yt:channelId>${feedChannelId}</yt:channelId>`)&&body.includes(`https://www.youtube.com/channel/${EXPECTED_YOUTUBE_CHANNEL_ID}`);const title=(body.match(/<author>\s*<name>([^<]+)<\/name>/i)||[])[1]||'';const ok=channelOk&&clean(title).toUpperCase()==='ZEVANORY';return result(true,ok,ok?'identity_match_public_feed':r.ok?'identity_mismatch':'provider_http_'+r.status,{channel_id:channelOk?EXPECTED_YOUTUBE_CHANNEL_ID:'',title:clean(title),verification_mode:'public_provider_feed'});}catch{return result(true,false,'public_feed_unavailable',{verification_mode:'public_provider_feed'});}
  }
