...
export function createCreativeSpec({offerId='OFFER-0001',channel='instagram',hook='',body='',cta='',objective='awareness',width=null,height=null,layout='editorial',formatName='default',campaignId='',variantId=''}={}){
  const offer=resolveCheckoutOffer(offerId); if(!offer) throw new Error('creative_offer_unknown');
  const key=String(channel).toLowerCase(); const [defaultWidth,defaultHeight]=dims[key]||[1080,1080];
  const spec={version:2,brand:'ZEVANORY',channel:key,objective:cap(objective,60),
    width:boundedDimension(width,defaultWidth),height:boundedDimension(height,defaultHeight),
    layout:cap(layout||'editorial',32),format_name:cap(formatName||'default',32),
...
export function verifyCreativeToken(payload,sig,env=process.env){
  ...
  const s=parsed.spec;
  const rebuilt=createCreativeSpec({offerId:s.offer_id,channel:s.channel,hook:s.hook,body:s.body,cta:s.cta,objective:s.objective,width:s.width,height:s.height,layout:s.layout,formatName:s.format_name,campaignId:s.campaign_id,variantId:s.variant_id});
...
export function creativeAssetEtag(spec,format='png'){
  return `"ci-${digest({creative_id:spec?.creative_id||'',format:String(format).toLowerCase(),renderer:'v2'}).slice(0,32)}"`;
}
