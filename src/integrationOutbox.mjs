...
export function buildOutboxEvent({aggregateType,aggregateId,eventType,destination,payload={},headers={},idempotencyKey}={}){
...
export async function enqueueOutbox(sql,event){
  ...
  const rows=await sql.query(`insert into integration_outbox(event_id,aggregate_type,aggregate_id,event_type,destination,payload,headers,idempotency_key)
    ...
    returning *`,[e.event_id,e.aggregate_type,e.aggregate_id,e.event_type,e.destination,JSON.stringify(e.payload),JSON.stringify(e.headers),e.idempotency_key]);
  return rows[0];
}
...
export async function dispatchOutboxOnce(sql,adapters={}){
  ...
  try{
    ...
    return Object.freeze({ok:true,processed:true,event_id:event.event_id,status:'delivered',result});
  }catch(error){
    ...
    await sql.query(`update integration_outbox set status=$2,last_error=$3,
      available_at=case when $2='retry' then now()+($4::text||' milliseconds')::interval else available_at end
      where event_id=$1`,[event.event_id,terminal?'dead_letter':'retry',String(error?.message||'delivery_failed').slice(0,500),delay]);
    return Object.freeze({ok:false,processed:true,event_id:event.event_id,status:terminal?'dead_letter':'retry',retry_in_ms:terminal?null:delay});
  }
}
