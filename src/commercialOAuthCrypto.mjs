...
export function openOAuthSession(value,env=process.env){const p=JSON.parse(decryptCommercialSecret(value,env));if(!p||typeof p!=='object'||Array.isArray(p)||!Number.isSafeInteger(p.iat)||p.iat>Date.now()||Date.now()-p.iat>=10*60*1000)throw new Error('commercial_oauth_session_expired');return p;}
