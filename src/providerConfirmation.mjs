...
export async function applyProviderConfirmation(sql,confirmation={}){
  ...
  const rows=await sql.query(`update integration_outbox set headers=jsonb_set(coalesce(headers,'{}'::jsonb),'{provider_confirmation}',$4::jsonb,true)
    ...
    returning event_id,run_id,headers`,[destination,messageId,occurredAtMs,JSON.stringify({provider,provider_event_id:eventId,provider_message_id:messageId,status,rank,occurred_at_ms:occurredAtMs,outcome:confirmationOutcome(status),confirmed_at:new Date().toISOString()}),rank]);
  return Object.freeze({updated:rows.length===1,event:rows[0]||null});
}
