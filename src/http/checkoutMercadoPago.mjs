...
export default async function handler(req,res){
  ...
  try{
    ...
    let raw; try{raw=await createMercadoPagoPreference(payload,providerToken);}catch{await sql.query(`UPDATE orders SET status='checkout_uncertain',updated_at=now() WHERE order_id=$1`,[order.order_id]);return json(res,503,{error:'checkout_provider_uncertain',accepted:false});}
    const checkout=normalizeMercadoPagoPreference(raw,order.external_reference,checkoutEnv); if(!checkout){await sql.query(`UPDATE orders SET status='checkout_uncertain',updated_at=now() WHERE order_id=$1`,[order.order_id]);return json(res,503,{error:'checkout_response_invalid',accepted:false});}
