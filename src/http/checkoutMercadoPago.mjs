...
export default async function handler(req,res){
  ...
  } const effectiveOffer=pilot?.authorized?Object.freeze({...input.offer,price_brl:certificationPilotAmountBrl(process.env)}):input.offer; const orderId=crypto.randomUUID(); const externalReference=externalReferenceForOrder(orderId);
  try{
    ...
    const payload=buildMercadoPagoPreference(order.order_id,publicBase,effectiveOffer); if(!payload)return json(res,503,{error:'checkout_payload_unavailable'});
