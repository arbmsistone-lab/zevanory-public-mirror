...
export default async function handler(req,res){
  ...
  try{
    const inserted=await sql.query(`INSERT INTO orders (order_id,request_id,session_id,experiment_id,offer_id,amount,currency,provider,external_reference,status,certification_pilot,certification_pilot_invite_id) VALUES ($1,$2,$3,$4,$5,$6,'BRL','mercadopago',$7,'created',$8,$9) ON CONFLICT (request_id) DO NOTHING RETURNING order_id`,[orderId,input.requestId,input.sessionId,PROJECT.experimentId,PROJECT.offerId,PROJECT.experimentalPriceBrl,externalReference,Boolean(pilot?.authorized),pilot?.invite_id||null]);
