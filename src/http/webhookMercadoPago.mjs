...
export default async function handler(req,res){
  ...
  try{
    ...
    if(Number(outcome.inserted_count)===1){
      try{await queueOutcomeLearningReview(sql,{idempotencyKey:`learning:financial:${providerEventId}`,source:`financial:${event.normalized}`});}catch{}
      if(event.normalized==='payment_confirmed'){
        try{await recordVerifiedLifecycleEvidence(sql,{dimension:'payment',source_class:'provider_webhook',source:'mercadopago',subject_ref:String(orders[0].order_id),idempotency_key:`payment-evidence:mercadopago:${providerEventId}`,metadata:{provider:'mercadopago'}});}catch{}
        try{await recordVerifiedLifecycleEvidence(sql,{dimension:'reconciliation',source_class:'provider_webhook',source:'mercadopago',subject_ref:String(orders[0].order_id),idempotency_key:`reconciliation-evidence:mercadopago:${providerEventId}`,metadata:{provider:'mercadopago'}});}catch{}
      }
    }
