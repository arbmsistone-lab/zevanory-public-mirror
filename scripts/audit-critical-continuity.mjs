...
const head=execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(); const gh=remote('https://github.com/arbmsistone-lab/ZEVANORY.git'); const gl=remote('https://gitlab.com/arbm-sistone/ZEVANORY.git');
add('GitHub loss survivable',gl===head,gl); add('GitLab loss survivable',gh===head,gh); add('dual git quorum',gh===gl&&gh===head);
...
const artifact=execFileSync('node',['scripts/public-artifact-recovery.mjs'],{encoding:'utf8'}); add('public artifact recovery',artifact.includes('PUBLIC_ARTIFACT_RECOVERY PASS'));
...
const failed=checks.filter(x=>!x.ok).length; console.log(`CRITICAL_CONTINUITY total=${checks.length} pass=${checks.length-failed} fail=${failed}`); if(failed)process.exit(1);
