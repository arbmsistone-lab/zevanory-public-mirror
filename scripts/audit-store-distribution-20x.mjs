...
for(let i=1;i<=20;i++){
  ...
  const ok=ZEVANORY_PRODUCTS.length===5&&
    catalogProduct?.sku==='ZEV-NGC-011'&&catalogProduct?.primary===false&&catalogProduct?.portfolio_role==='content_bundle'&&catalogProduct?.pilot_price_brl===347&&/^[0-9a-f]{64}$/i.test(catalogProduct?.artifact_sha256||'')&&
