...
const handoff=readFileSync(new URL('../launch/ZEVANORY-PRODUCTS-V20-HANDOFF.md',import.meta.url),'utf8');
let pass=0;
for(let i=1;i<=20;i++){
  ...
  const ok=ZEVANORY_PRODUCTS.length===5&&product?.version==='2.0'&&product?.primary===false&&product?.portfolio_role==='content_bundle'&&product?.pilot_price_brl===347&&product?.artifact_materialized===true&&product?.content_quality_certified===true&&product?.quality_certified===false&&product?.sellable===false&&/^[0-9a-f]{64}$/i.test(product?.artifact_sha256||'')&&checkout===null&&blocked.ready===false&&blocked.public_download===false&&blocked.blockers.includes('payment_not_reconciled')&&handoff.includes('Produtos digitais v2.0')&&handoff.includes('sellable=false');
