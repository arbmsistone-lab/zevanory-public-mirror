...
add('03 required schema tables',REQUIRED_TABLES.length>=23&&['integration_outbox','agent_jobs','agent_runs','agent_memory','agent_control_state','agent_approvals','customer_lifecycle_profiles','customer_lifecycle_events','attribution_touchpoints','lifecycle_evidence_events','lifecycle_certification_artifacts','certification_pilot_invites','intelligence_snapshots'].every(x=>REQUIRED_TABLES.includes(x)));
add('04 required migrations',REQUIRED_MIGRATIONS.length>=16&&REQUIRED_MIGRATIONS.includes('011_agent_control_and_trace')&&REQUIRED_MIGRATIONS.includes('012_sales_lifecycle_v2')&&REQUIRED_MIGRATIONS.includes('015_lifecycle_certification_provenance')&&REQUIRED_MIGRATIONS.includes('024_market_product_intelligence')&&await exists('db/migrations/015_lifecycle_certification_provenance.sql')&&await exists('db/migrations/024_market_product_intelligence.sql')&&await exists('db/migrations/016_certification_pilot.sql')&&dr.includes('015_lifecycle_certification_provenance.sql')&&dr.includes('016_certification_pilot.sql'));
