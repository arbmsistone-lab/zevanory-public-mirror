...
const plan=await text('src/activationPlan.mjs'); const api=await text('api/activation-readiness.mjs');
...
add('03 activation API exists',await exists('api/activation-readiness.mjs'));
add('04 activation API syntax',run('--check','api/activation-readiness.mjs'));
...
add('09 service payment requirements mapped',Boolean(ACTIVATION_REQUIREMENTS.asaas_production_not_configured)&&Boolean(ACTIVATION_REQUIREMENTS.asaas_credentials_missing));
...
add('15 API is read only',api.includes("req.method!=='GET'")&&!api.includes('sql.query')&&!api.includes('fetch(')&&!api.includes('execFile')&&!api.includes('spawn'));
...
add('17 route published',vercel.includes('/api/activation/readiness'));
add('18 release requires activation route',release.includes('/api/activation/readiness'));
add('19 env template separates DR and Gemini',env.includes('DR_REHEARSAL_ALLOWED=false\nGEMINI_API_KEY='));
