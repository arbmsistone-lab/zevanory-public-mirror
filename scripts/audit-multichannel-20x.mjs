...
const checks=[
 ...
 ['owned web present',!!CHANNELS.zevanory],['whatsapp present',!!CHANNELS.whatsapp],['email present',!!CHANNELS.email],
 ['instagram present',!!CHANNELS.instagram],['facebook present',!!CHANNELS.facebook],['tiktok present',!!CHANNELS.tiktok],
 ['youtube present',!!CHANNELS.youtube],['linkedin present',!!CHANNELS.linkedin],['google SEO present',!!CHANNELS.google],
 ['affiliate present',!!CHANNELS.affiliate],['professional email domain',PROFESSIONAL_EMAIL.primary==='contato@zevanory.api.br'],
 ['email aliases defined',PROFESSIONAL_EMAIL.aliases.length===3],['email auth DNS required',PROFESSIONAL_EMAIL.requiredDns.join(',')==='MX,SPF,DKIM,DMARC'],
 ['all profiles tracked',Object.values(CHANNEL_PROFILES).every(x=>x.url.includes('utm_campaign=arbm_sist_launch'))],
...
let ok=0; for(const [name,pass] of checks){console.log(`${pass?'APPROVED':'FAILED'} ${name}`); if(pass) ok++;}
console.log(`AUDIT_MULTICHANNEL_20X_${ok===20?'APPROVED':'FAILED'} units=20 approved=${ok} failed=${20-ok}`); if(ok!==20) process.exit(1);
