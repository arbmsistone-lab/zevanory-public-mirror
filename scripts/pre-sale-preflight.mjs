...
const customDomain=await probeCustomDomain();const result=evaluatePreSaleReadiness({
  ...
  mercadopago_token:Boolean(process.env.MERCADOPAGO_ACCESS_TOKEN),
  mercadopago_webhook:Boolean(process.env.MERCADOPAGO_WEBHOOK_SECRET),
  mercadopago_env_sandbox:String(process.env.MERCADOPAGO_ENV||'').trim().toLowerCase()==='sandbox',
