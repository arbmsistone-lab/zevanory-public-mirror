...
const probes=await text('src/statusProbes.mjs'); const status=await text('api/status.mjs'); const assurance=await text('src/http/assurance.mjs'); const probe=await text('scripts/operational-probe.mjs'); const vercel=JSON.parse(await text('vercel.json'));
