...
add('08 release EG0038',release.r.status===200&&release.body.release_id==='ZEVANORY-EG0038-FINAL'&&assurance.r.status===200,release.body.release_id);
...
add('13 public status has no PII keys',![status.text,agent.text].some(x=>/contact_ref|session_id|phone|email/i.test(x)));
add('14 commercial gates remain off',config.body.commercial_enabled===false&&config.body.whatsapp_enabled===false&&agent.body.commercial_execution==='blocked');
