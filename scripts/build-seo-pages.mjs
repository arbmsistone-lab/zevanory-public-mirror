...
const schema=(p,url)=>JSON.stringify({
  ...
  description:p.desc,url,image,sku:p.sku,brand:{'@type':'Brand',name:'ZEVANORY'},provider:{'@type':'Organization','@id':base+'/#organization',name:'ZEVANORY',url:base}
});
...
for(const p of products){
  ...
  if(!p.customPage) writeFileSync(join(root,`${p.slug}.html`),shell(`${p.name} | ZEVANORY`,p.desc,url,body,schema(p,url)),'utf8');
}
...
writeFileSync(join(root,'solucoes.html'),shell('Soluções ZEVANORY | IA, automação e software','Conheça o portfólio oficial ZEVANORY: ZEVANORY ONE, ARBM SIST, IA na Prática, Vendas na Prática, Lucro & Caixa e soluções integradas.',`${base}/solucoes`,portfolioBody,portfolioSchema),'utf8');
