...
const files = [
  ...
  '010_payment_provider_abstraction.sql','011_agent_control_and_trace.sql','012_sales_lifecycle_v2.sql','013_lifecycle_evidence_certification.sql','014_lifecycle_evidence_trust.sql','015_lifecycle_certification_provenance.sql','016_certification_pilot.sql',
  '017_mercadolivre_oauth.sql','018_tiktok_oauth.sql','019_tiktok_sandbox_provider.sql','020_private_artifact_delivery.sql','021_linkedin_nuvemshop_oauth.sql',
];
...
try {
  ...
  result = assessSchemaIntegrity({tableNames:tables.rows.map(row=>row.table_name),migrationIds:migrations.rows.map(row=>row.migration_id)});
...
console.log(JSON.stringify({ok:true,mode:'transactional-rollback',schema_rebuild:result,persistent_changes:false,rehearsal_rto_ms:Date.now()-startedAt,rto_scope:'schema-rebuild-rehearsal-not-production-contract'}));
