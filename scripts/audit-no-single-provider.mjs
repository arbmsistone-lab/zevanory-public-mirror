...
const provider=process.env.GITLAB_CI?'gitlab':process.env.BITBUCKET_BUILD_NUMBER?'bitbucket':process.env.GITHUB_ACTIONS?'github':process.env.CIRCLECI?'circleci':'operator';
const eventSha=process.env.CI_COMMIT_SHA||process.env.BITBUCKET_COMMIT||process.env.GITHUB_SHA||process.env.CIRCLE_SHA1||'';
...
const providerUrls={
  ...
  gitee:process.env.ZEVANORY_GITEE_REPO_URL||'',
