...
add('08 channels and outbound default to canonical gate with no production bypass',channels.includes('gateEvaluator=salesGate')&&outbound.includes('commercialGate=salesGate')&&outbound.includes('ensureOutboundAllowed(event,env,commercialGate)')&&outbound.includes('supportEventAllowed')&&outbound.includes('organicEventAllowed')&&agentRun.includes('buildOutboundAdapters()'));
