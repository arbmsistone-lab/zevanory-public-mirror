...
import { SALES_LIFECYCLE_CANONICAL_V2, salesLifecycleGate } from '../src/salesLifecycleV2.mjs';
...
add('03 environment flags alone cannot unlock current sales gate',salesLifecycleGate().approved===false);
...
add('08 channels and outbound default to canonical gate with no runtime override',channels.includes('salesGate(env)')&&outbound.includes('commercialGate=salesGate')&&agentRun.includes('buildOutboundAdapters()'));
add('09 certification also requires audit parity and release approval',salesLifecycleGate().blockers.includes('lifecycle_audit_10x_not_passed')&&salesLifecycleGate().blockers.includes('production_parity_not_verified')&&salesLifecycleGate().blockers.includes('lifecycle_release_not_approved'));
