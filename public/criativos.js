...
function renderPreview(){
  ...
  if(!url){if(loading)loading.textContent='Preview indisponível.';return;}
  if(mode==='image'){
...
function renderSummary(){
  ...
  const sales=Boolean(closure?.commercial_enabled);set('kpi-publish',sales?'LIBERADA':'BLOQUEADA');set('truth-state',sales?'PUBLICAÇÃO LIBERADA':'SEM PUBLICAÇÃO');
  set('truth-copy',sales?'Publicação comercial está autorizada pelos gates atuais.':'A Central mostra criação e avaliação reais; publicação comercial segue bloqueada.');
  set('engine-state',sample?.engine?'MOTOR ONLINE':'MOTOR INDISPONÍVEL');set('production-count',`${(sample?.image_variants?.length?1:0)+(sample?.video_variants?.length?1:0)} linhas`);
  ...
  set('flow-state',sales?'MOTOR + PUBLICAÇÃO':'MOTOR ATIVO · PUBLICAÇÃO BLOQUEADA');
}

function selectMode(next){
  mode=next;document.querySelectorAll('.production-item').forEach(el=>el.classList.toggle('active',el.dataset.mode===mode));renderPreview();
}
...
async function load(){
  try{
    ...
    renderSummary();renderPreview();set('source-state','fonte real · creative_sample conectado');
  }catch(error){
...
document.querySelectorAll('.production-item').forEach(el=>el.addEventListener('click',()=>selectMode(el.dataset.mode)));
