(()=>{
  ...
  const source=(location.pathname.split('/').filter(Boolean)[0]||'zevanory').replace(/[^a-z0-9_-]/gi,'').slice(0,40);
  const text=`Ola, vim pela ${source} da ZEVANORY e quero atendimento.`;
