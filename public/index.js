...
function renderPublicOperations(status={},config={},agent={}){
  ...
  set('live-connection-state',Array.isArray(agent.autopilot?.timeline)&&agent.autopilot.timeline.length?'PROVA REAL':'SEM TRACE'); const x=document.getElementById('live-connection-state');if(x)x.dataset.state=Array.isArray(agent.autopilot?.timeline)&&agent.autopilot.timeline.length?'ready':'blocked';
...
function renderLiveProof(agent={}){
  const auto=agent.autopilot||{}, timeline=Array.isArray(auto.timeline)?auto.timeline:[];
  const latest=timeline[0]||null; set('live-proof-routine',auto.enabled?'ATIVA · '+fmt(auto.cadence_minutes)+' MIN':'SEM PROVA');
  set('live-proof-cycle',latest?.cycle_id||auto.latest?.cycle_id||'SEM PROVA'); set('live-proof-trace',latest?.trace_id?String(latest.trace_id).slice(0,13)+'…':'SEM PROVA');
  ...
  for(const x of timeline.slice(0,18)){const row=document.createElement('article');row.className='proof-row';const t=document.createElement('time');t.textContent=liveTime(x.created_at);const body=document.createElement('div');const b=document.createElement('b');b.textContent=stageLabels[x.stage]||x.stage||'Evento';const small=document.createElement('small');const d=x.details||{};small.textContent=[x.cycle_id,d.subject,d.verified_sources!=null?d.verified_sources+' fontes':null,d.candidate_id,d.best_channel,d.quality_score!=null?'Q '+Number(d.quality_score).toFixed(2):null].filter(Boolean).join(' · ');body.append(b,small);const proof=document.createElement('span');proof.className='proof-id';proof.textContent=x.run_id?String(x.run_id).slice(0,8):'SEM ID';row.append(t,body,proof);root.appendChild(row);}
}
