...
function renderTimeline(data){
  ...
  for(const r of data.runs||[]){
    const row=document.createElement('div'); row.className='event'; row.dataset.state=stateClass(r.outcome);
...
function render(data){
  ...
  renderTimeline(data);renderCapabilities(data);renderGates(data);renderJourney();renderHealth(data);renderApprovals(data);$('updated').textContent=`Atualizado ${new Date(data.generated_at||Date.now()).toLocaleString('pt-BR')}`;
}
...
async function connectOperator(){const token=$('operator-token').value.trim();if(!token){$('auth-error').textContent='Informe o token de operador.';return false;}try{const data=await fetchOperatorState(token);operatorToken=token;$('operator-token').value='';$('mode-label').textContent='MODO OPERADOR AUTENTICADO';$('connection').textContent='TELEMETRIA REAL';$('robot-substate').textContent='Dados operacionais reais sem PII.';render(data);return true;}catch(e){$('auth-error').textContent=String(e.message||'Falha de autenticação.');return false;}}
async function togglePause(){if(!operatorToken)return;const action=lastData.control?.paused?'resume':'pause';const r=await fetch('/api/robot-control-action',{method:'POST',headers:{authorization:`Bearer ${operatorToken}`,'content-type':'application/json'},body:JSON.stringify({action,reason:`robot_control_${action}`})});if(!r.ok)return;render(await fetchOperatorState());}
async function decidePendingApproval(approvalId,decision){if(!operatorToken)return;const r=await fetch('/api/robot-approval',{method:'POST',headers:{authorization:`Bearer ${operatorToken}`,'content-type':'application/json'},body:JSON.stringify({approval_id:approvalId,decision,reason:`robot_control_${decision}`})});if(!r.ok)return;render(await fetchOperatorState());renderApprovals(lastData);}
